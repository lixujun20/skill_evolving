...
from academic.benchmarks.core.credit_events import apply_credit_evidence, normalize_credit_events
...
class SpreadsheetMaintenanceAdapter(NoOpMaintenanceAdapter):
    ...
    async def run_task(
        ...
        *,
        ...
        return await _compat_module().run_spreadsheet_task(
            task,
            llm_config=config.llm_config,
            model_name=config.model_name,
            artifact_store=store,
    ...
    async def run_micro_maintenance(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        async def run_bundle_test(**hook_kwargs: Any) -> Dict[str, Any]:
            artifact = hook_kwargs["artifact"]
            if not artifact.bundle.all_cases():
                return {
                    "skill_name": artifact.name,
                    "passed": True,
...
def _spreadsheet_micro_targets(
    *,
    ...
    return micro_target_names(
        relevant_skill_names=[artifact.name for artifact in extracted] + explicit,
        credit_events=credit_events,
        credit_bundle_cases=credit_bundle_cases,
    )
    ...
    *,
    ...
    for case in artifact.bundle.all_cases():
        ...
        if not task.input_artifacts.get("input_xlsx") or not task.expected.get("golden_xlsx"):
            ...
            continue
        result = await _compat_module().run_spreadsheet_task(
            task,
            llm_config=config.llm_config,
            model_name=config.model_name,
