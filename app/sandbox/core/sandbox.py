...
class DockerSandbox:
    ...
    def __init__(
        ...
        self.extra_terminals: Optional[Dict[AsyncDockerizedTerminal]] = {}

    async def create(self, env_vars=None) -> "DockerSandbox":
        ...
        try:
            ...
            host_config = self.client.api.create_host_config(
                ...
                network_mode="none" if not self.config.network_enabled else "host",
            ...
            if not env_vars:
                env_vars = {}
            env_vars.update({"PYTHONUNBUFFERED": "1"})
            # Initialize terminal
    ...
    async def create_terminal(self, terminal_id, env_vars=None, start_commands=None):
        """Creates a new terminal instance for the sandbox.

        Returns:
            New terminal instance.

        Raises:
            RuntimeError: If sandbox not initialized or terminal creation fails.
        """
        if not self.terminal:
            raise RuntimeError("Sandbox not initialized")
        assert terminal_id != 0, 'terminal id 0 is reserved for the main terminal.'
        assert not terminal_id in self.extra_terminals, 'terminal id {} already exists.'.format(terminal_id)
        ...
        await terminal.init()
        if start_commands:
            assert isinstance(start_commands, list) and all(isinstance(command, str) for command in start_commands)
            for command in start_commands:
                await terminal.run_command(command, timeout=60)
        if self.extra_terminals is None:
            self.extra_terminals = {}
        self.extra_terminals[terminal_id] = terminal

    async def has_terminal(self, terminal_id):
        return terminal_id in self.extra_terminals or terminal_id == 0
    ...
    async def run_command(self, cmd: str, timeout: Optional[int] = None, terminal_id: Optional[int] = None) -> str:
        ...
        if not self.terminal:
            raise RuntimeError("Sandbox not initialized")
        if terminal_id is None or terminal_id == 0:
            terminal = self.terminal
        elif terminal_id > len(self.extra_terminals):
            raise RuntimeError("Terminal id invalid. Should be less than {}".format(len(self.extra_terminals) + 1))
        else:
            terminal = self.extra_terminals[terminal_id]
        try:
        ...
        try:
            if self.terminal:
                try:
                    ...
                    self.terminal = None
                if self.extra_terminals:
                    for terminal in self.extra_terminals.values():
                        try:
                            await terminal.close()
                        except Exception as e:
                            errors.append(f"Extra terminal cleanup error: {e}")
                    self.extra_terminals = {}
    ...
    async def close_terminal(self, terminal_id):
        terminal = self.extra_terminals[terminal_id]
        await terminal.close()
        del self.extra_terminals[terminal_id]
