...
class DockerSession:
    ...
    async def execute(self, command: str, timeout: Optional[int] = None) -> str:
        ...
        try:
            ...
            # Build a sentinel that bash can echo back, so read_output() can detect end-of-command.
            # IMPORTANT: this shell is bash, NOT Python — use `echo`, not `print()`.
            command_sentinel_value = f"__CMD_DONE_{hash(sanitized_command) & 0xFFFFFFFF}__"
            full_command = f"{sanitized_command}\necho '{command_sentinel_value}'\n"
            ...
            async def read_output() -> str:
                ...
                while True:
                    ...
                    try:
                        ...
                        # if buffer.endswith(b"$ ") or buffer.endswith(b">>> ") or re.search(rb'In \[\d+\]: $', buffer):
                        #     print('prompt found break')
                        #     print('buffer: {}'.format(buffer.decode('utf-8')))
                        #     break
                        if command_sentinel_value.encode() in b'\n'.join(lines):
                            # print('command sentinel found break')
                            break
                ...
                result_lines = [x for x in result_lines if command_sentinel_value.encode() not in x]
